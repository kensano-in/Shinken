// ⚫ SHINCHAT — BLACKHOLE V2 (INSANE ANIMATION LEVEL) // Next.js + Tailwind + Advanced Motion Effects

'use client' import { useEffect, useState } from 'react'

export default function BlackholeComingSoon() { const [time, setTime] = useState(0) const [statusIndex, setStatusIndex] = useState(0) const [glitch, setGlitch] = useState(false)

const status = [ "Booting neural core...", "Injecting consciousness...", "Warping spacetime layers...", "ShinChat awakening..." ]

useEffect(() => { const timer = setInterval(() => setTime(t => t + 1), 1000)
...
{/* MULTI LAYER BLACKHOLE */}
  ...
  {/* TITLE WITH GLITCH */}
  <h1 className={`text-6xl md:text-8xl font-extrabold text-center mb-6 tracking-widest relative ${glitch ? 'animate-glitch' : ''}`}>
    SHINCHAT
  </h1>

  <p className="text-gray-400 text-center mb-2 tracking-[0.3em] animate-fade">
    ENTERING THE SYSTEM
  </p>
  ...
  <div className="flex gap-4">
    <input
      ...
      className="px-6 py-3 bg-black border border-red-500 rounded-lg focus:shadow-[0_0_25px_rgba(255,0,0,0.7)] outline-none transition-all"
    />
  ...
  <div className="absolute bottom-8 text-center text-gray-500">
    ...
    <div className="flex gap-6 text-xs">
      <span>support@shinken.in</span>
      <span>@shinken.in</span>
      <span>@shinichiro</span>
      <span>Shinken</span>
    </div>
  </div>
...
function Box({ label, value }) { return ( <div> <p className="text-4xl font-bold">{value}</p> <span className="text-gray-500 text-xs">{label}</span> </div> ) }
